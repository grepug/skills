...
This repo also publishes example GitHub issue templates under `.github/ISSUE_TEMPLATE/` so issue-driven workflow patterns can be copied without reverse-engineering a consuming repo.
...
| `inline-doc-governance` | Audit and govern inline documentation coverage for repo-owned TypeScript, JavaScript, and Swift files, including file headers, type docs, contract docs, generated-file exclusions, and reusable repo policy setup | Source files to audit, repo guidance to discover local policy, and Python 3 for the deterministic audit helper | Draft |
...
| `macos-dev-storage-cleanup` | Measure reclaimable macOS developer storage, generate an explicit cleanup plan, apply only approved cleanup items, and verify final disk space | macOS for live cleanup, Python 3 for bundled scripts, and explicit user approval before deletion | Draft |
