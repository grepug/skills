## Summary

{{SUMMARY_LINES}}

## Linked Issue

{{CLOSES_LINE}}

## Closeout Checklist

- [x] Re-read the linked issue after implementation
- [x] Reconciled the canonical plan comment checklist against the shipped code
- [x] No unchecked implementation checklist items remain in the issue body or canonical plan comment
- [x] {{INLINE_DOC_AUDIT_LINE}}
- [x] This PR inherits the linked issue milestone when one exists

## Checklist Audit

- Issue body unchecked items: {{ISSUE_OPEN_COUNT}}
- Canonical plan comment blocking unchecked items: {{PLAN_BLOCKING_OPEN_COUNT}}
- Canonical plan comment non-blocking unchecked items: {{PLAN_NON_BLOCKING_OPEN_COUNT}}
- Canonical plan comment: {{PLAN_COMMENT_LINK}}

## Validation

{{VALIDATION_LINES}}
