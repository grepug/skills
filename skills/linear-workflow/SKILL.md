...
Write Linear issues and comments in the target repo's designated Linear language. Discover that from repo guidance such as `AGENTS.md`, contributor docs, workflow docs, or the user's explicit instruction. If no Linear-specific language is defined, default to English. Linear language may differ from GitHub issue, PR, code, or general repo working language.
...
Use PM mode when the input is product intent, source material, PRD content, discussion notes, a roadmap item, or a request to shape PM-facing work. Read `references/pm-mode.md`.
...
When a detail is unclear or a developer finds a flaw in the product contract, comment in Linear instead of resolving it only in chat or GitHub. Read `references/discussion.md`.
...
3. Determine the designated Linear language. Default to English when repo guidance does not specify one.
...
8. Create or update the Linear issue in PM language, using the designated Linear language.
...
- Linear content written in the repo's designated Linear language, or English when none is defined
